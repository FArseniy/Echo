...
function getWebRtcConfiguration({ allowTurn = false } = {}) {
  ...
  if (!host) return { enabled: false, iceServers: [] };

  const iceServers = [{ urls: [`stun:${host}:${stunPort}`] }];
  if (allowTurn && sharedSecret) {
    ...
    iceServers.push({
      urls: [`turn:${host}:${stunPort}?transport=udp`, `turn:${host}:${stunPort}?transport=tcp`, `turns:${host}:${tlsPort}?transport=tcp`],
      username,
      credential: crypto.createHmac('sha1', sharedSecret).update(username).digest('base64'),
    });
  }
  return { enabled: true, iceServers };
}
