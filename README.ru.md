...
## Режим доставки сообщений

По умолчанию Echo пытается передать сообщение напрямую между браузерами через WebRTC DataChannel:

```
браузер A ─── P2P/WebRTC ─── браузер B
       \──── Socket.IO ────/  сигналинг и резерв
```

- STUN находит прямой маршрут;
- TURN используется автоматически, только когда прямой маршрут недоступен;
- если WebRTC не установился, сообщение доставляется через Socket.IO;
- отправитель видит статус: «Доставлено N из M».

Сервер создаёт, проверяет и временно хранит сообщения для истории, поэтому этот режим не является сквозным шифрованием (E2EE). TURN-учётные данные короткоживущие и не передаются в URL.
